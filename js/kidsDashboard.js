document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadPlaylists() {
        try {
            console.log('Cargando playlists para el perfil:', currentProfile.full_name);
            console.log('PIN utilizado:', currentProfile.pin);
            
            // Obtenemos el token del administrador para mayor compatibilidad
            const token = localStorage.getItem('token');
            
            const response = await fetch('http://localhost:3000/api/restricted/playlists', {
                method: 'GET',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Error al cargar las playlists: ${response.status} ${response.statusText}`);
            }
    ...
    function renderPlaylists(playlists) {
        ...
        playlists.forEach(playlist => {
            ...
            sectionHeader.innerHTML = `
                ...
                <span class="badge bg-primary rounded-pill video-count">${playlist.videoCount || 0} videos</span>
            `;
    ...
    async function loadVideosForPlaylist(playlistId, container) {
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/restricted/videos?playlistId=${playlistId}`, {
                method: 'GET',
                headers: {
                    'x-restricted-pin': currentProfile.pin,
                    'Authorization': `Bearer ${token}` // Añadimos también el token del administrador
            ...
            // Log para depuración
            console.log('Respuesta de API de videos:', response.status, response.statusText);
    ...
    async function playVideo(videoId, playlistId) {
        try {
            ...
            currentVideoIndex = currentPlaylistVideos.findIndex(v => v._id === videoId);
